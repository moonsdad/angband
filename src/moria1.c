...
/* can't print AC here because might be in a store */
    ...
    if (TR2_IM_FIRE & item_flags2) p_ptr->flags.fire_im = TRUE;
    if (TR2_IM_ACID & item_flags2) p_ptr->flags.acid_im = TRUE;
    if (TR2_IM_COLD & item_flags2) p_ptr->flags.cold_im = TRUE;
    if (TR2_IM_LIGHT & item_flags2) p_ptr->flags.light_im = TRUE;
    if (TR2_IM_POIS & item_flags2) p_ptr->flags.poison_im = TRUE;
    if (TR2_RES_ACID & item_flags) p_ptr->flags.acid_resist = TRUE;
    if (TR2_RES_LIGHT & item_flags) p_ptr->flags.lght_resist = TRUE;
    if (TR2_RES_FIRE & item_flags) p_ptr->flags.fire_resist = TRUE;
    if (TR2_RES_COLD & item_flags) p_ptr->flags.cold_resist = TRUE;
    if (TR_POISON & item_flags) p_ptr->flags.poison_resist = TRUE;
    if (TR2_RES_CONF & item_flags2) p_ptr->flags.confusion_resist = TRUE;
    if (TR2_RES_SOUND & item_flags2) p_ptr->flags.sound_resist = TRUE;
    if (TR2_RES_LT & item_flags2) p_ptr->flags.light_resist = TRUE;
    if (TR2_RES_DARK & item_flags2) p_ptr->flags.dark_resist = TRUE;
    if (TR2_RES_CHAOS & item_flags2) p_ptr->flags.chaos_resist = TRUE;
    if (TR2_RES_DISENCHANT & item_flags2) p_ptr->flags.disenchant_resist = TRUE;
    if (TR2_RES_SHARDS & item_flags2) p_ptr->flags.shards_resist = TRUE;
    if (TR2_RES_NEXUS & item_flags2) p_ptr->flags.nexus_resist = TRUE;
    if (TR2_RES_BLIND & item_flags2) p_ptr->flags.blindness_resist = TRUE;
    if (TR2_RES_NETHER & item_flags2) p_ptr->flags.nether_resist = TRUE;
    if (TR2_RES_FEAR & item_flags2) p_ptr->flags.fear_resist = TRUE;
