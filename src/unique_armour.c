int unique_armour(inven_type *t_ptr)
{
	...
	if (wizard || peek)
	    ...
	    good_item_flag = TRUE;
	t_ptr->flags2 |= (TR2_RES_ACID | TR2_RES_COLD);
	t_ptr->flags2 |= (TR2_HOLD_LIFE | TR2_RES_CHAOS | TR2_RES_DARK |
	...
	t_ptr->flags2 |= (TR2_RES_FIRE | TR2_RES_COLD | TR2_RES_ACID | TR2_RES_POIS |
			  TR2_IM_ELEC | TR2_RES_LITE | TR2_RES_ELEC | TR2_FREE_ACT | TR_ARTIFACT);
	t_ptr->flags3 |= (TR3_ACTIVATE | TR3_LITE | TR3_HIDE_TYPE | TR3_SEE_INVIS | TR3_AGGRAVATE);
	t_ptr->toac = 25;
	t_ptr->pval = -2;
	t_ptr->weight = 400;
	t_ptr->ac = 30;
	...
	if (wizard || peek)
	    ...
	    good_item_flag = TRUE;
	t_ptr->flags1 |= (TR1_DEX | TR1_SEARCH | TR3_REGEN);
	t_ptr->flags2 |= (TR2_RES_FIRE | TR2_RES_COLD | TR2_RES_ACID | TR2_RES_POIS |
			 TR2_RES_ELEC | TR2_HOLD_LIFE | TR2_RES_CONF | TR2_RES_SOUND | TR_RES_LITE
	      | TR2_RES_DARK | TR2_RES_CHAOS | TR2_RES_DISENCHANT | TR_ARTIFACT
	       | TR2_RES_SHARDS | TR2_RES_BLIND | TR2_RES_NEXUS | TR2_RES_NETHER);
	t_ptr->flags3 |= (TR3_HIDE_TYPE | TR3_ACTIVATE);
	t_ptr->toac = 35;
	t_ptr->pval = -3;
	t_ptr->ac = 50;
	t_ptr->tohit = -4;
	...
	if (wizard || peek)
	    ...
	    good_item_flag = TRUE;
	t_ptr->flags1 |= (TR1_CON | TR1_STR | TR1_SEARCH);
	t_ptr->flags2 |= (TR2_RES_SOUND | TR2_RES_CHAOS | TR_ARTIFACT | TR2_IM_ACID | TR2_FREE_ACT |
			 TR2_RES_ACID);
	t_ptr->flags3 |= TR3_HIDE_TYPE;
	...
	if (wizard || peek)
	    ...
	    good_item_flag = TRUE;
	t_ptr->flags1 |= (TR1_DEX | TR2_FREE_ACT);
	t_ptr->flags2 |= (TR2_RES_ACID | TR_ARTIFACT);
	t_ptr->flags3 |= (TR3_SHOW_MODS | TR3_ACTIVATE);
	...
	switch (randint(6)) {
	  ...
	  default:
	    ...
	    t_ptr->flags1 |= (TR1_STR | TR1_DEX);
	    t_ptr->flags2 |= (TR_ARTIFACT);
	    t_ptr->flags2 |= (TR3_SHOW_MODS | TR3_AGGRAVATE | TR3_CURSED);
	...
	t_ptr->flags2 |= (TR2_RES_ACID | TR2_RES_FIRE | TR2_RES_COLD |
			 TR2_RES_ELEC | TR_ARTIFACT);
	t_ptr->flags3 |= (TR3_ACTIVATE);
	...
	t_ptr->flags2 |= (TR2_RES_ACID | TR2_RES_FIRE | TR2_RES_COLD | TR2_RES_LITE |
			 TR2_RES_DISEN | TR2_RES_DARK | TR_ARTIFACT);
	t_ptr->flags3 |= (TR3_ACTIVATE);
	...
	if (wizard || peek)
	    ...
	    good_item_flag = TRUE;
	t_ptr->flags1 |= (TR1_STR | TR1_CON | TR1_DEX);
	t_ptr->flags2 |= (TR_ARTIFACT | TR2_RES_NEXUS | TR2_RES_ACID);
	...
	if (randint(6) == 1) {
		...
		good_item_flag = TRUE;
	    t_ptr->flags2 |= (TR2_RES_ACID | TR2_RES_FIRE | TR2_RES_COLD | TR2_RES_LITE);
	    t_ptr->flags1 |= (TR1_CON | TR1_DEX | TR1_STR);
	    t_ptr->flags2 |= (TR3_TELEPATHY | TR3_LITE |  | TR3_SEE_INVIS);
	    t_ptr->flags2 |= (TR2_RES_LITE | TR2_RES_BLIND
			      | TR_ARTIFACT);
	    t_ptr->name2 = ART_DOR;
	    t_ptr->pval = 4;
	    t_ptr->toac = 20;
	    ...
	    t_ptr->flags2 |= (TR2_RES_BLIND | TR_ARTIFACT | TR2_RES_ACID);
	    t_ptr->name2 = ART_HOLHENNETH;
	    t_ptr->flags3 |= (TR3_HIDE_TYPE | TR3_SEE_INVIS | TR3_ACTIVATE);
	    t_ptr->pval = 2;
	    t_ptr->toac = 10;
	    t_ptr->cost = 100000L;
	    HOLHENNETH = 1;
	    ...
	    t_ptr->flags3 |= (TR3_HIDE_TYPE | TR3_SEE_INVIS | TR3_CURSED
			     | TR3_AGGRAVATE);
	    t_ptr->pval = -125;
	    t_ptr->toac = 10;
	    t_ptr->cost = 0L;
	...
	t_ptr->flags1 = (TR1_STR | TR1_CON | TR1_WIS);
	t_ptr->flags2 |= (TR2_RES_LITE | TR2_RES_BLIND | TR_ARTIFACT
			| TR2_RES_ACID | TR2_RES_FIRE);
	t_ptr->flags3 = (TR3_ACTIVATE | TR3_LITE | TR3_SEE_INVIS | TR3_REGEN);
	t_ptr->pval = 3;
	t_ptr->toac = 15;
	t_ptr->cost = 100000L;
	GONDOR = 1;
	...
	t_ptr->flags2 |= (TR2_RES_ACID| TR2_FREE_ACT | TR_ARTIFACT);
	t_ptr->flags3 |= (TR3_TELEPATHY | TR3_SEE_INVIS | TR3_CURSED );
